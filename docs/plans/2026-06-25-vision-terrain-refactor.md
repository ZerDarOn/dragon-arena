...
对照源码复核后，原计划有四处"把已完成的事当成待办"或"病因诊断错误"，照原文实施会白干甚至改坏。下文各章保留原貌作背景，但**以本节为准**：

1. **§3 视野计算 = 后端已全部实现，不要重写。**
   `vision_service.py:40-196` 已实现扇形裁剪、Bresenham 挡墙/挡烟、光源扩散、黑暗视觉、黑暗过滤、`compute_detected_tokens`（仅方位），并已接入 `state_sync`（`handler.py:869, 904-905`）。**§3 视为"现状文档/附录"，不是构建任务。** 本次重构真正的范围 = 前端渲染 + 迷雾记忆 + `darkness_strength` + 协议合并 + 改 bug。

2. **§3.2/§5 "Room 类型已有" 表述误导。**
   `visible_cells`/`detected_tokens` **不是 `Room` 模型字段**，是每个观察者各自计算、塞进各自 `state_sync` 的临时值（`handler.py:904-905`）；前端 `types.ts:89-90` 才有。**别给后端 Room schema 加这俩字段，也别持久化。**

3. **§7 病因诊断错误（已在源头修复 ✅）。**
   实测：环境暗涂在格子底色（`GameCanvas.vue:337`），token 在后续循环单独绘制并已按可见性过滤（`:385-388`）——暗**不会**盖住 token。真正的 bug 在后端：站暗格时 `compute_visible_cells` 把暗的 `origin` 也过滤掉了，导致玩家在黑暗中从自己视野消失。**已修：`vision_service.py` 现在始终保留 `origin`**，并加了回归测试 `test_self_cell_visible_even_when_dark`。§7 的前端特例可不做。

4. **§6 聊天昵称 = 3 处都要改（已完成 ✅）。**
   `MessageBubble.vue` 有三个分支显示 `sender_id`（骰子/空间语音/普通），原文只改 1 处。**已修：三处统一走 `senderName` computed**（`sender_id` 确为 player_id，`room.players` 查得到）。

**其它小修：**
- token 状态图标**已实现**（`GameCanvas.vue:431-438`），§4.2 勿重复造。
- §3.1 Step5"烟雾格本身不可见"是**改规则行为**（现状是烟雾只挡穿过的视线），动前先对设计文档 ADR。
- §10 离屏 canvas 缓存对 900 格属过度优化，先不做。
...
> ⚠️ **本章已在 `vision_service.py` 完整实现（见 §0.5 第 1 条）。以下为现状参考，不是待办。**
...
## 6. 聊天用户名修复  ✅ 已完成（3 处分支统一改，见 §0.5 第 4 条）
...
## 7. 黑暗中的 Token 可见性修复  ✅ 已在后端源头修复（病因见 §0.5 第 3 条，下文原诊断有误）
...
### Phase A — 紧急修复（不涉及架构）
- [x] A1. 聊天用户名显示修复（3 处分支统一 senderName）
- [x] A2. ~~Token 渲染顺序~~ → 真因是后端暗格过滤掉 origin，已在 `compute_visible_cells` 修复 + 回归测试
- [ ] A3. 暗区笔刷视觉反馈修正（选"黑暗"画笔时，给一个明确的视觉标记，别只改元数据）
